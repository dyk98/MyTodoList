...
function buildEmptyTodoTemplate(year) {
    return `# ${year} TODO\n\n## 待办池\n\n---\n`;
}
async function getTodoFileStatus(userEmail, targetYear) {
...
function buildTodoGuardPayload(status) {
...
async function readTodoFileWithGuard(res, userEmail, targetYear, allowAutoCreate) {
...
async function ensureTodoFileExists(res, userEmail, targetYear) {
    const status = await getTodoFileStatus(userEmail, targetYear);
    if (!status.exists) {
        const guard = buildTodoGuardPayload(status);
        res.status(guard.statusCode).json({
...
// 获取当前年 TODO 文件状态
app.get('/api/todo/status', optionalAuthMiddleware, async (req, res) => {
    try {
        const userEmail = req.user?.email || null;
        const currentYear = new Date().getFullYear();
...
// 创建新年度 TODO 文件（需要登录）
app.post('/api/todo/create-year', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/todo/move', authMiddleware, async (req, res) => {
    try {
        const { year, fromLineIndex, toLineIndex, position } = req.body;
        const targetYear = resolveYear(year);
        if (typeof fromLineIndex !== 'number' || typeof toLineIndex !== 'number') {
            return res.status(400).json({ success: false, error: 'fromLineIndex and toLineIndex are required' });
        }
        ...
        const getIndentLen = (line) => {
            const match = line.match(/^(\s*)/);
            return match ? match[1].length : 0;
        };
        const stripIndent = (line) => line.replace(/^\s*/, '');
        const collectSubtreeLines = (startIndex) => {
        ...
        const findSubtreeEnd = (startIndex) => {
