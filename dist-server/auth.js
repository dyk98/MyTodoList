...
async function writeInvites(data) {
    await fs.writeFile(INVITES_FILE, JSON.stringify(data, null, 2), 'utf-8');
}
// ========== 用户目录管理 ==========
export function getUserDataDir(email) {
...
export function getUserDocsDir(email) {
    return path.join(getUserDataDir(email), 'docs');
}
async function createUserDirectories(email) {
...
export async function login(email, password) {
    ...
    const token = jwt.sign({ email: user.email, isAdmin: user.isAdmin }, JWT_SECRET);
...
// ========== 邀请码管理 ==========
export async function generateInviteCode(adminEmail, targetEmail) {
    // 验证管理员权限
    const usersData = await readUsers();
