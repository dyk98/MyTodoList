...
interface TaskMeta {
  ...
  completed: boolean
  originalLine: string
  hasCompletedDescendants: boolean
...
function buildTaskTreeFromPool(lines: string[]): Map<string, TaskNode[]> {
  const projectTrees = new Map<string, TaskNode[]>()
  let currentProject = ''
  let inPool = false
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inPool && isTaskLine(line)) {
      ...
      const node: TaskNode = { meta, children: [] }
...
// 递归标记任务状态（自底向上）
function markTaskStatus(node: TaskNode): void {
  // 递归处理所有子节点
...
// 构建归档内容（递归）
function buildArchiveLines(node: TaskNode, baseIndent: number): string[] {
  const lines: string[] = []
...
// 构建待办池保留内容（递归）
function buildRetainedLines(node: TaskNode): string[] {
  const lines: string[] = []
...
// 替换待办池内容
function replacePoolInLines(lines: string[], newPoolContent: string[]): string[] {
...
app.post('/api/todo/week-settle', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // ========== 阶段 3 - 构建归档内容（用于周区块） ==========
    const archiveContents: { projectName: string; lines: string[] }[] = []
    for (const [projectName, roots] of projectTrees) {
      const projectArchiveLines: string[] = []
    ...
    // ========== 阶段 4 - 重建待办池（保留未完成任务） ==========
    const newPoolLines: string[] = ['## 待办池', '']

    for (const [projectName, roots] of projectTrees) {
      const projectRetainedLines: string[] = []
    ...
    // 将归档内容转换为旧格式（兼容后续代码）
    const completedTasks: { lines: string[]; projectName: string }[] = archiveContents.map(ac => ({
...
app.post('/api/todo/set-today', authMiddleware, async (req: AuthRequest, res) => {
  try {
...
// 移除今日任务标记
app.delete('/api/todo/remove-today', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { lineIndex, date, year } = req.body
    const targetYear = resolveYear(year)
