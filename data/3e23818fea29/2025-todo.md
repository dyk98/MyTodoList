...
- [ ] 采集评测集上 SOTA 模型的数据
    - [ ] gpt 5.2-codex-xhigh
- [x] 沟通GetRolloutTaskResult 数据不足问题
...
- [x] MyTodoList
    - [x] 拖拽一个任务到另一个任务下成为子任务
    - [x] 支持直接 MarkDown 模式修改待办池，及按回车不要确定
- [x] AgentRL
    - [x] 使用 claude、codex 无头模式，然后只给我们注入的 tool，蒸馏对话轨迹，可以使用 whistle 代理，直接抓全部对话轨迹
    - [x] 分析 1215-sft 及 12160-grpo 整体对话轨迹以及筛掉编译不通过部分的对话轨迹
    - [x] qwen3-30b GRPO
        - [x] 跑通 GRPO 流程
        - [x] 定义新的奖励函数
        - [x] 获取产物中的对话轨迹进行分析，完整 GRPO 训练
    - [x] 合并 Driver 最新的变动到 RL 分支
...
- [x] 学习任务
    - [x] LLM推理优化 - Prefill-Decode分离式推理架构 https://mp.weixin.qq.com/s/FwgYf3yHLM6wDHaalj9Saw
    - [x] 约束解码 https://zhuanlan.zhihu.com/p/14675125571
    - [x] AgentEvolver https://arxiv.org/html/2511.10395v1
    - [x] retool 论文 https://arxiv.org/pdf/2504.11536
- [x] AgentRL
    - [x] 使用优质大参数模型针对数据集进行规模采样，获取优质对话轨迹用于SFT
    - [x] 迭代场景，去除多余无用的 tool
    - [x] 对用来做 SFT 的 qwen3-coder-30b 做一下对话轨迹分析 sample 的  qwen3-coder-30b-driver-sampling
    - [x] 对 qwen3-coder-30b 和 hy2 的数据进行一波统一的数据清洗，得到一个质量更高的数据集 - 以每个 devtoolsGenerateProject.json 为样本单位，先用 history 统计工具调用错误，再按阈值过滤。   - 过滤指标：       - totalErrors：FunctionResponse 的错误次数（所有工具一视同仁）。       - consecutiveErrors：同一工具连续报错序列的最大长度。       - overallRepeatRate：全局相邻两次 functionCall 参数完全相同的比例。       - sequenceRepeatRate：连续错误序列中“完全重复重试 / 全部重试”的比例。       - errorRate：错误数 / 工具交互轮数。       - syntaxErrors、paramErrors：按字符串匹配的高风险错误计数。       - openaiError：有 API error 直接剔除。       - badOutcomes：连续错误序列若是 abandoned/continuedError/(strict 下还含 switchedTool) 则剔除
- [x] 教育场景
    - [x] 去除无用 tool 提升创建速度
- [x] AgentDriver
    - [x] 整体数据采集、分析、优化方案
...
- [x] 教育场景
    - [x] 编写完整的并发提升文档
        - [x] 对 MiniMax 最新部署的实例做压测
        - [x] 随机捞线上的 N 条对话轨迹，在原有 agent 下进行性能测试,然后在 Agent 下进行性能测试做对比
    - [x] 填写昆仑芯测试文档
    - [x] 集中修复教育场景的 MiniMax 适配问题
- [x] MyTodoList
    - [x] 支持移动端访问
    - [x] 创建新用户没创建docs
    - [x] 修复周结算的时候破坏了我原本的层级结构，直接将其打平了
    - [x] 修复周结算没有跟着最新的区块创建
    ...
    - [x] 修复移动端便利贴样式问题
    - [x] 给项目起名字+生成 logo
    - [x] 线上不使用npm run dev.换成pm2
- [x] 学习任务
    - [x] 1. 大模型基础知识 pdf
    - [x] 基础概念学习
        - [x] scaling，scaling laws
        - [x] 参数、训练超参数、推理超参数
        - [x] loss 损失函数
        - [x] 监督学习，强化学习，RLHF-人类反馈强化学习 先让人工标注者对多种候选回答打分或排序，再训练一个"奖励模型"模拟人工偏好。随后通过强化学习（比如 PPO）调参，使模型倾向于产生奖励模型更高分的回答，从而更符合人类直觉与价值观。
        - [x] 预训练、后训练和 SFT
- [x] AgentRL
    - [x] SFT 前后细粒度能力提升对比
        - [x] 对 SFT 后 496 条数据对话轨迹进行分析，提取错误 driver-test-qwen3-4b-1204-from-scratch-v6-120415
        - [x] 对 SFT 前 496 条数据进行对话轨迹分析，提取错误 driver-test-qwen3-4b-base-from-scratch-v6-120310
...
- [x] AgentRL
    - [x] 去掉错误重试
    - [x] 增加 10 分钟请求超时的限制
    - [x] 解决超时时间异常的问题：undici Agent 的 headersTimeout / bodyTimeout / connect.timeout 对于快速响应的服务>器不生效（可能是 undici 的实现问题或特性），使用AbortSignal.timeout()解决
- [x] AgentDriver
    - [x] 针对 MiniMax M2 的并发数据测试，得到并发测试总结
    - [x] 排查教育场景模型异常中断问题->网关问题
    - [x] 设计教育整体数据分析和自动优化文档
- [x] 教育场景
    - [x] 教育 prompt 调整修复标签问题
    - [x] 需要压测一下单实例的 TPM，设计一个压测任务
    - [x] 确认灰度策略以及申请对应的显卡资源用来部署 MiniMax M2
- [x] 其他
    - [x] AI账号采买
    - [x] 保险采购
- [x] MyTodoList
