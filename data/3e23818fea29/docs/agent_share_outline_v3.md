...
- **小程序流程示意**：
  ...
  ```
- **解决的问题**：
  - LLM 终于成为可控的 API，我们可以设定配额、参数、权限；脚本节点则负责真实执行，避免人工复制/粘贴。
...
- **tips**: 虽然 qwen-code 是 fork 自 GeminiCLI，但是基于代码实现" ≠ "交互范式完全一致，qwen-code 把默认 Loop 改成更贴合 Qwen-Coder 的 ReAct 风格，重新打磨了工具 schema、prompt 结构和 CLI 交互。

#### 主流范式
- **ReAct（Reason + Act）**：qwen-code、Claude computer-use 的默认模式，透明可回放，适合多次观察-修正；缺点是 verbose 且思维链暴露，需要额外治理。
- **Plan + Execute**：Gemini 系的代表，擅长长流程任务但高度依赖前期规划，一旦 plan 偏了就一路偏。
- **CodeAct**：用可执行 Python 代码替代 JSON/Text 作为动作空间，复用 LLM 代码预训练能力，天然支持复杂控制流，OpenHands、Codex CLI 等项目已采用；缺点是对非代码任务不够直观。
- **Auto Tool-use**：如 AutoGen ToolAgent、企业版 qwen Agent，模型自行判断何时调用工具，适合多工具调度，但最容易乱用或卡死。
- **Memory-based**：Claude-code 长上下文+记忆型推理，强调上下文延续，交互体验佳但容易跑偏，需要记忆治理（正是这个原因他们很早就搞了 CLAUDE.md）。

#### 即使两个 Agent 使用同一种范式，实际体验也会天差地别
...
#### 共性缺陷与选型困境
- **领域上下文缺失**：不管是哪一派 Agent，脱离业务上下文都无法直接落地，只能靠人不断补素材、补规则，于是诞生了诸如 rule、agents.md 等方法。
- **范式迭代太快**：今年大家追 ReAct，明年又流行 Plan-and-solve。跟着每一波范式/产品改架构，成本极高。
- **难以统一治理**：不同 Agent 的 Prompt、工具、日志格式各不相同，换一个 Agent 就得重做监控、安全、费用分析。
- **决策困难**：我们究竟用哪种方法？即便选好了，也担心下一波热潮又得推翻重来。

## 5. 阶段 4：自主 Agent 展望
- **未来可能的自主型 Agent 方向**：
  - **多 Agent 分工协作**：不同 Agent 各司其职（编码、测试、部署、运营），通过某种调度机制实现任务拆分、上下文同步、冲突调解。
  - **自主规划与自主学习**：面对开放任务能自动拆分子目标、排列优先级、在失败后给出自检结论并主动求助。
  - **自主管理记忆**：能分辨哪些信息要长期记忆、哪些是短期 buffer、哪些该遗忘，并能主动清理与压缩。
  - **自主运行**：不靠人类轮询，能自己监控、触发、执行、反馈，像后台服务一样持续运转，发现事件就行动。
  - **更丰富的环境交互**：不仅会调 API，还能直接操作桌面、浏览器、移动端设备，甚至外部硬件。
- **行业趋势观察**：
  - 模型能力持续提升，工具调用的可靠性和推理深度都在快速进化。
  - MCP（Model Context Protocol）等标准化协议正在推动工具生态的互通。
  - 长上下文、多模态能力让 Agent 能处理更复杂的真实场景。
  - Human-in-the-loop 仍是当前阶段的必要保障，但介入频率在逐步降低。
- **值得关注的方向**：
  - 如何沉淀可复用的"慢变量"（工具协议、场景资产、治理规则），让业务在范式快速迭代时仍能稳定复用。
  - 如何平衡 Agent 的自主性与可控性，在效率和安全之间找到合适的边界。
  - 如何构建有效的反馈闭环，让 Agent 从执行结果中持续学习和优化。

## 6. 总结 & Q&A
- **带走的两件事**（回扣全程的"小程序自动化"主线）：
...
- Codex 之所以"刚刚好"，主要因为它的设计**高度吻合 GPT5 系列模型在 RLHF/RLAIF 与 tool-use 训练中的典型交互模式**。
